...
def read_table(table_name):
    ...
    return df
@st.cache_data(ttl=600)
def load_and_prepare_data(sample_size=3000):
    transactions = read_table("Transactions")
    products = read_table("Products")
    households = read_table("Households")

    # Sample a subset of transactions
    transactions_sampled = transactions.sample(n=sample_size, random_state=42)

    # Merge data
    df = transactions_sampled.merge(products, on="product_num").merge(households, on="hshd_num")
...
# ------------------ BASKET ML ------------------
    ...
    if df_dept.shape[0] < 10:
        ...
        # Train model
        model = train_model(features, df_dept["target"])
